...
## 2026-09-25 세션 (이어서 4): SW1/SW2 대시보드에 파이프라인 갱신 지연(stale) 경고 추가

**배경**: 두 대시보드 모두 "자동화 파이프라인이 실제로 살아있는지" 알려주는 표시가 없었음.
SW2는 실시간 페이퍼 트레이딩 데이터가 "언제 기준인지" 표시 자체가 없었고, SW1은 기준 시각은
보이지만 오래돼도 경고가 뜨지 않음. GitHub Actions 스케줄이 조용히 실패해도 날짜를 직접 계산해
보지 않는 한 알 방법이 없었음. 전략 로직은 전혀 건드리지 않는 순수 진단/UX 기능이라 바로 진행.
(브라우저 연결이 끊겨 새 작업으로 이관된 뒤 이어서 마무리함.)

**설계**: `collect-daily-data.yml`(22:00 UTC)/`run-paper-trading.yml`(22:30 UTC)은 평일(Mon-Fri)만
실행 → 정상 최대 공백은 금→월 주말 3일. `STALE_THRESHOLD_DAYS = 4`(휴일/CI 지연 여유 1일)로 두어
정상 주말은 조용히 넘어가고, 4일을 초과하면(5일째부터) 상태 점이 노란색(amber)으로 바뀌며
"⚠ 데이터 갱신 N일째 지연 — …" 문구 표시. fetch 자체가 실패하면 빨간색(error)으로 구분.
일수 계산은 UTC 날짜 단위로 해서 자정 부근 off-by-one 흔들림 방지.

**변경 내용**: 2개 파일, 2커밋.
- `docs/index.html` (`110e91c`): `STALE_THRESHOLD_DAYS`/`pipelineDaysStale(Date)` 추가,
  `status-dot`에 id 부여, `.status-dot.stale`/`.status-dot.error` CSS 추가, `loadSignals()`에서
  `run_ts` 기준 지연 판정 → dot 클래스 토글 + 문구 접두어, `.catch`에서 error 클래스 부여.
- `docs/sw2.html` (`45e8bc1`): 동일 상수 + 날짜 문자열("YYYY-MM-DD") 파싱 버전
  `pipelineDaysStale()` + `setPipelineStatus(text, state)` 헬퍼 추가, 헤더 brand-row에
  status-pill 마크업 신규 추가(기존 CSS 재사용), 동일 CSS 2블록 추가, `renderEquityChart()`의
  3갈래 분기(전부 실패 → error / 데이터 없음 → "첫 실행 대기" / 성공 → 마지막 equity 날짜 기준
  판정)에 각각 상태 설정.

**검증**:
- jsdom으로 두 파일 각각 7개 시나리오(당일/2일/3일/4일 경계/5일/10일/fetch 실패) — 14건 전부
  PASS, JS 에러 없음. 4일까지는 조용, 5일부터 경고, 실패는 error로 구분됨을 확인.
- 참고: 이전 세션 로컬 작업본이 유실돼 인계 메모의 앵커 설명대로 pristine에서 재구성함 — 따라서
  인계 메모의 "최종 목표 SHA-256"과는 다르고(문구 세부 차이), 대신 재구성본을 jsdom으로 재검증한
  뒤 브라우저 에디터 문서 해시가 로컬 테스트본과 일치(index `76d141d5…`, sw2 `b487f0e3…`)함을
  확인하고 커밋. pristine 해시는 인계 메모와 일치(그 사이 다른 커밋 없었음).
- 2커밋 모두 GitHub Actions 체크 4종(`pytest`, `build`, `deploy`, `report-build-status`) `success`.
- 실 배포 대시보드 확인: 두 페이지 모두 콘솔 에러 없음, 새 코드 반영됨. 데이터가 신선하므로 초록
  점 + 경고 없음이 정상 — SW1 "2026년 9월 25일 금 오전 07:06 (KST) 기준 최신 데이터",
  SW2 "2026-09-24 장 마감 기준 최신 데이터", equity 차트 정상 렌더링.
...
## 2026-09-25 세션 (이어서 5): SW2 일일 실행 멱등성 버그 수정 — 중복/주말 equity 행 제거 (PR #5)

**배경**: 현우님 요청("SW 문제점을 스스로 찾아 더 나은 방향으로 고치기")으로 자체 점검 중 발견.
`data/sw2/equity/*.csv`에 실제 거래일 9일(09-14~09-24)에 대해 행이 16개 — 09-14가 5번, 09-13/09-16이
2번씩, 토(09-12)·일(09-13) 행까지 있었음. 원인: `run_daily_paper_trading.py`가 `run_date`를 벽시계
UTC 날짜로 잡고, 같은 날 재실행(수동 `workflow_dispatch`)이나 주말 실행을 막는 장치가 없었음. 결과적으로
일간 수익률 관측치의 약 40%가 가짜 0.0 → t-test, block bootstrap, 거버넌스 판정이 모두 왜곡됨. 또한 같은
날 재실행 시 방금 익절/손절로 청산한 종목을 다시 매수(buy_1은 "미보유"만 확인)할 수 있는 위험도 있었음.

**변경 내용** (브랜치 `fix/sw2-idempotent-runs`, PR #5 — 4파일이 서로 의존해 단일 파일 커밋으로는 중간
상태 CI가 깨지므로 PR로 묶음):
- `sw2/ledger.py`: `mark_to_market()`이 같은 날짜면 덮어쓰기(append 대신). `equity_df()`는 날짜당 마지막
  값만 남기고 토/일 제거 — 읽기 단계 정리일 뿐, 저장된 `equity_curve` 원본은 그대로 둠(되돌리기 가능).
- `scripts/run_daily_paper_trading.py`: `resolve_run_date()` — SW1 시그널 파일의 `date` 컬럼(시장 날짜)을
  사용, 없을 때만 벽시계 날짜로 폴백. `already_processed()` — 이미 처리된 날짜면 해당 모델 매매 스킵.
- 테스트: `write_signals()` 헬퍼가 호출마다 다음 영업일을 찍도록 변경(= 매 `main()` 호출이 실제처럼 서로
  다른 거래일). 신규 6건 — 재실행 no-op, 당일 청산 후 재진입 방지, 시장 날짜 run_date, 폴백, ledger
  중복/주말 제거.

**검증**:
- 로컬 전체 pytest 341 passed (기존 335 + 신규 6).
- 실제 `data/` 사본으로 드라이런: 5개 모델 모두 "2026-09-24 already processed -- skipping trades",
  equity CSV가 깨끗한 9거래일로 재생성, 비교 통계는 모델당 실제 수익률 n=8 사용. 데이터 파일은 PR에
  포함하지 않음 — 머지 후 다음 스케줄 실행이 자동 재생성.
- PR #5 CI pytest success. 전략/스코어링/임계값 로직 변경 없음.

**자체 점검에서 발견했지만 보류한 것 (사용자 판단 필요)**:
- `baseline`·`conservative` 모델이 가동 2주 동안 한 번도 매매하지 않음(자산 100,000 그대로). 최근 M7
  `quant_score` 최대값이 0.31(AMZN)이라 통합 점수가 `buy_1_score`(0.30)에 뉴스 가중 후 거의 닿지 않음.
  매매 없는 모델은 비교 통계에 정보를 주지 못함 — 임계값/가중치 조정은 스코어링 로직 변경이라 보류 중.

**다음 세션이 할 일**: PR #5 머지 여부 확인(머지 전엔 오늘 밤 스케줄 실행이 여전히 구 코드로 돎). 머지 후
첫 스케줄 실행에서 equity CSV가 중복 없는 거래일만 담는지, 비교 JSON의 n 값이 거래일 수-1과 맞는지 확인.
