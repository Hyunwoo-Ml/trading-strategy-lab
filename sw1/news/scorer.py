...
STATUS: interface + parsing/aggregation logic are complete and tested with
mocked LLM responses. The actual Anthropic API call (`call_anthropic`) is
wired up but requires ANTHROPIC_API_KEY, which is not registered yet --
score_news_for_ticker() raises AnthropicKeyMissing until it is. Once the
key is set (as a GitHub Actions secret, same pattern as the Toss/KIS keys),
